...
public class HEXKeyBoardManager {
    private Cal mCal;
    ...
    public static HEXKeyBoardManager get() {

        return SingletonHolder.instance;
    }

    private static class SingletonHolder {
        private static HEXKeyBoardManager instance = new HEXKeyBoardManager();
    }
    ...
    public void key(Context context, String command, String inputStr, EditText mInput) {
        ...
        //若输入正确，则将输入信息显示到显示器上
        if ("0123456789.()sincostanlnlogn!+-×÷√^ABCDEF".contains(command)
                && tip_lock) { //共25个按键
            print(command);
