...
public class CityMangerFragment extends BaseFragment implements OnPickListener, IAddressCallback, SwipeMenuCreator, OnItemMenuClickListener, IWeaCallback, ICityCacheCallback, IWeatherCallback {
    ...
    protected void intView() {
        ...
        iv_bar_add.setVisibility(View.VISIBLE);


        RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams)city_toolbar.getLayoutParams();
        layoutParams.topMargin= MyStatusBarUtil.getStatusBarHeight(getActivity());
        city_toolbar.setLayoutParams(layoutParams);

        //侧滑删除
        mSwipeRecyclerView.setSwipeMenuCreator(this);
        mSwipeRecyclerView.setOnItemMenuClickListener(this);
        LinearLayoutManager manager = new LinearLayoutManager(getActivity());
        RecyclerViewItemDistanceUtil.setDistance(mSwipeRecyclerView, getActivity(), 3.5f);
    ...
    protected void intEvent() {
        ...
        iv_bar_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                isAdd = false;
                CityPicker.from(CityMangerFragment.this)
                        .enableAnimation(false)
                        .setAnimationStyle(0)
                        .setLocatedCity(mLocatedCity)
                        .setHideBar(true)
                        .setOnPickListener(CityMangerFragment.this).show();
            }
        });
            ...
            public void deleteOnClick(CityCacheBean city, int position) {
                if (mWeaCachePresent != null && !TextUtils.isEmpty(mCurrentCity) && mCityCachePresent != null) {
                    if (city.getCity().equals(mCurrentCity)) {
                        RxToast.error(getActivity(), "当前所在城市不能移除").show();
                    } else {
    ...
    @Override
    public void onError() {
        RxToast.error(getActivity(), "没有网络，请重新再试").show();
    }
